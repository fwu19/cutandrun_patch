...
pubDir=$(realpath $( cat $json | jq '.outdir' | sed "s/\"//g; s/\'//g" ))
genome=$( cat $json | jq '.genome' | sed "s/\"//g; s/\'//g" )
gtf4homer=$(realpath $( cat $json | jq '.gtf4homer' | sed "s/\"//g; s/\'//g" ))
...
for peak in $(find $pubDir/$subdir/07_consensus_peaks/ | egrep "*.bed$"); do
    echo -e Annotate consensus peak,$(sbatch $slurm $shDir/annotate_bed.sh $peak $genome $gtf4homer),$(basename $peak)
done



# IFS=" " read -r -a peaks <<< "$(find $pubDir/$subdir/07_consensus_peaks/ | egrep "*.bed" |tr '\n' ' ' )"
# npeak=${#peaks[@]}
# echo -e Annotate consensus peak,$(sbatch $slurm --array=1-${npeak} --wrap="cd $(dirname ${peaks[ (( $SLURM_ARRAY_TASK_ID-1 ))]}); bash $shDir/annotate_bed.sh $(basename ${peaks[ (( $SLURM_ARRAY_TASK_ID-1 ))]}) $genome $gtf4homer" )
